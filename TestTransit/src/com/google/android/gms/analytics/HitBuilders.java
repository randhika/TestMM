...
public class HitBuilders
{
    ...
    protected static class HitBuilder
    {
        ...
        public HitBuilder addImpression(Product product, String s)
        {
            ...
            if (!wB.containsKey(s))
            {
                wB.put(s, new ArrayList());
            }
            ((List)wB.get(s)).add(product);
            return this;
        }
        ...
        public Map build()
        {
            ...
            Iterator iterator1 = wD.iterator();
            for (int j = 1; iterator1.hasNext(); j++)
            {
                hashmap.putAll(((Product)iterator1.next()).X(o.u(j)));
            }

            Iterator iterator2 = wB.entrySet().iterator();
            for (int k = 1; iterator2.hasNext(); k++)
            {
                java.util.Map.Entry entry = (java.util.Map.Entry)iterator2.next();
                List list = (List)entry.getValue();
                String s = o.x(k);
                Iterator iterator3 = list.iterator();
                for (int l = 1; iterator3.hasNext(); l++)
                {
                    hashmap.putAll(((Product)iterator3.next()).X((new StringBuilder()).append(s).append(o.w(l)).toString()));
                }
